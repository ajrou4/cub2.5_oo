...
/*   Updated: 2023/10/29 22:41:33 by majrou           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void generate3DProjection(t_map_data *mape, int __unused i)
{
    ...
    float projectedWallHeight = (SQ_ / perpDistance) * ((WIDTH / 2) / tan(mape->FOV/2));
        ...
        mape->heightwall = wallStripHeight;
        ...
        for (int j = 0; j < wallTopPixel; j++)
        {
          ...
          mlx_put_pixel(mape->img, i, j, ft_pixel(mape->color.floor[0], mape->color.floor[1], mape->color.floor[2], 0));
        }
        for (int j = wallBottomPixel; j < HEIGHT; j++)
        {
          if (i > 0 && i < HEIGHT && j > 0 && j < WIDTH )
          mlx_put_pixel(mape->img, i, j,  ft_pixel(mape->color.sky[0], mape->color.sky[1], mape->color.sky[2], 0));
        }
}
