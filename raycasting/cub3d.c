...
/*   Updated: 2023/10/29 22:43:50 by majrou           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void  DDA(t_map_data *map, int  X0, int Y0, int X1, int Y1)
{
	...
	float X = X0;
	float Y = Y0;
	float inc =  map->texteur->no->height / map->heightwall;
	float y_text = 0.0;
	if (map->heightwall > HEIGHT)
		y_text = ((map->heightwall - HEIGHT) / 2) * inc;
	float x_text = 0.0;
	for (int i = 0; i <= steps; i++)
	{
		...
		{
			x_text = ((map->vertWallHitY / SQ_) - (int)(map->vertWallHitY / SQ_)) * map->texteur->no->width;
			if (map->isRayFacingRight)
				mlx_put_pixel(map->img, X, Y, get_color_from_textur(map->texteur->no, x_text, y_text)); // put pixel at (X,Y)
			else
				mlx_put_pixel(map->img, X, Y, get_color_from_textur(map->texteur->no, x_text, y_text)); // put pixel at (X,Y)
		}
		else
		{
			x_text = ((map->horzWallHitX / SQ_) - (int)(map->horzWallHitX / SQ_)) * map->texteur->no->width;
...
void	filling(t_map_data *mape,int i, int j)
{
	...
	if (mape->cur_char == '0' || mape->cur_char == 'N')
